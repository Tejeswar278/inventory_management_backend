...
def delete_product_cont(data):
    ...
    product_to_delete = next((p for p in existing_category.products if p.name == product_name), None)

    if not product_to_delete:
        return jsonify({"message": f"Product '{product_name}' does not exist in category '{category_name}'"}), 404

    existing_category.products = [p for p in existing_category.products if p.name != product_name]
    existing_category.save()
